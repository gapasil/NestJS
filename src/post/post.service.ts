  ...
  async createPost(post, userId, files) {
    try {
      ...
      for (let key in files) {
        if (key == 'video') {
          const masNameVideo = this.fileService.createMultiFile(
            FileType.VIDEOPOST,
            files.video,
          );
        ...
        if (key == 'MassImgPost') {
          const masNameImgMas = this.fileService.createMultiFile(
            FileType.IMGPOSTMAS,
            files.MassImgPost,
          );
        ...
        if (key == 'imgPost') {
          const masNameImg = this.fileService.createMultiFile(
            FileType.IMGPOST,
            files.imgPost,
          );
        ...
        if (key == 'file') {
          const masNameFile = this.fileService.createMultiFile(
            FileType.FILEPOST,
            files.file,
          );
        ...
        if (key == 'audio') {
          const masNameAudio = this.fileService.createMultiFile(
            FileType.AUDIOPOST,
            files.audio,
          );
      ...
      const nameFile = await this.fileService.createJsonPost(
        FileType.JSON,
        JSON.stringify(postP),
      );

      let nameFileImg = 'null';

      if (files.imageView) {
        nameFileImg = this.fileService.createFile(
          FileType.IMGTITLE,
          files.imageView[0],
        );
      }
  ...
  async getPostQery(req) {
    ...
    if (req.body.qery.hasOwnProperty('view')) {
      return (Posts = await this.PostModel.find({
        $and: [
          req.body.qery.idUser
            ? { idUser: req.body.qery.idUser }
            : { idUser: { $ne: 1 } },
          req.body.qery.date?.minDate && req.body.qery.date?.maxDate
            ? {
                date: {
                  $gt: req.body.qery.date.minDate,
                  $lt: req.body.qery.date.maxDate,
                },
              }
            : { idUser: { $ne: 1 } },
          req.body.qery.theme
            ? { theme: req.body.qery.theme }
            : { idUser: { $ne: 1 } },
          req.body.qery.language
            ? { languagePost: req.body.qery.language }
            : { idUser: { $ne: 1 } },
        ],
      })
        .sort({ view: -1, date: -1 })
        .clone()
        .limit(req.body.value));
    } else {
      return (Posts = await this.PostModel.find({
        $and: [
          req.body.qery.idUser

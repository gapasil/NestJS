...
export class AuthService {
  ...
  async login(user): Promise<any> {
    ...
    const token = await bcrypt
      .compare(user.password, candidate.password)
      .then((res) => {
        if (!res) {
          throw new HttpException(
  ...
  async registration(user, avatar): Promise<string> {
    try {
      ...
      if (avatar.avatar) {
        nameFile = this.fileService.createFile(
          FileType.AVATAR,
          avatar.avatar[0],
        );
      }
      ...
      if (user.email) {
        setTimeout(async () => {
          emailCodeErr = await this.mailerService.mailer(
            user,
            `${process.env.URL}auth/activateemail/${activationLinkEmail}`,
          );

          if (emailCodeErr) {
            if (emailCodeErr.responseCode == 501) {
              throw new HttpException(
                'Не удалось доставить сообщение такого почтового ящика не существует!',
                HttpStatus.BAD_REQUEST,
              );
            }

            this.usersService.deleteUser({
              activationLinkEmail: activationLinkEmail,
            });
            return;
          }
        }, 2000);
      ...
      return JSON.stringify({
        message: 'Переидите в почту которую указали при регистраций',
        token: token.token,
      });
    } catch (e) {
      console.log(e);
  ...
  async generateToken(user): Promise<any> {
    ...
    return {
      token: this.jwtService.sign(payload, {
        secret: process.env.TOKEN_SECRET, // Секретный ключ
        expiresIn: '1h', // Пример срока действия
      }),
    };
  }
}
